...
class BadgeVC: UIViewController , SGPagingTitleViewDelegate, SGPagingContentViewDelegate {
    ...
    func pagingContentView(index: Int) {
        if index == 1 {
            DispatchQueue.main.async {
                self.pagingTitleView.removeBadge(index: 1)
            }
        } else if index == 2 {
            DispatchQueue.main.async {
